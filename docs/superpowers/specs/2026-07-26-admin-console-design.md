...
7. S2 — 管理者パスワード変更（`PUT /api/admin/password`。5.3 参照）
   ※ S4 で初期パスワードをランダム化したため、変更手段が無いと発行された値を
   一生使い続けることになる。API のみ実装し、UI は Phase 2 の `adminSystem` 画面で作る。

**Phase 2**（運営機能）
8. `src/migrator.py` ＋ `migrations/0005_admin_console.sql` ＋ 適用UI
9. `src/admin_api.py` / `public/admin.js` への切り出し（機能追加なし、純粋な移設）
10. 管理者アカウント管理（一覧 / 追加 / 削除）と `adminSystem` 画面
    （S2 のパスワード変更 API は Phase 1 で実装済み。その画面もここで作る）
11. 代理閲覧
12. 店舗ライフサイクル（設定編集 / アーカイブ / エクスポート / 完全削除）
13. 全社ダッシュボード
14. 監査ログ強化（フィルタ / ページング / CSV）
15. 一斉通知
16. E2E テスト

手順9（切り出し）を機能追加より前に置くのは、移設と機能変更を同じコミットに混ぜると
差分レビューが不可能になるため。
