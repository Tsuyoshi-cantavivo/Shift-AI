...
- **確定シフトの直接編集は UI＋サーバの二重ロック。** UI は確定シフトのバータップでメモのみ編集可能なモーダルを表示。サーバ側 `PUT /api/shop/shifts/<id>` も、既存が `confirmed` で時刻・担当が変わる編集は `409 locked` で拒否する（時刻・担当を変えない再保存、`requested`/`modifying` からの確定は許可）。内部用エスケープとして `allow_confirmed_edit=true` を用意（UI は送らない）。時間変更・取消は「変更申請の承認」で反映する。
- **監査ログの `created_at` は JST。** 他テーブルは `datetime('now')`(UTC) だが、監査ログは `jst_now()` で JST 記録し、管理画面の表示と一致させる。
- **日またぎの超過集計は今回対象外。** 超過検知は既存 `_check_slot_cap` と同じ「開始日基準の拡張スロット」モデルを踏襲する。カレンダー日境界をまたいだ超過の合算は、スケジューリングエンジンと cap 判定が共有するスロットモデルの変更を伴い影響が大きいため、別途の検討事項とする。
