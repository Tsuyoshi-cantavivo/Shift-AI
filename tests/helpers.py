...
def insert_wish_overnight(shop_id, staff_id, start_day, start_time, end_day, end_time,
                          availability=None, note="テスト"):
    """wish_history と shifts.requested に日またぎ希望を INSERT（テスト用）。

    insert_request/insert_wish は end 時刻を start と同じ日付で保存するため、
    22:00〜翌05:00 のような日またぎ希望には使えない。こちらは start_day と
    end_day を別々に指定できる。
    """
    sd = f"{start_day}T{start_time}:00"
    ed = f"{end_day}T{end_time}:00"
    try:
        dbmod.execute(
            "INSERT INTO wish_history (shop_id, staff_id, start_datetime, end_datetime, availability, note) "
            "VALUES (?,?,?,?,?,?)",
