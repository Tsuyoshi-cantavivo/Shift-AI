...
# 「22-2」のような日またぎ表記として認める最大の実働時間。
# これを超える組み合わせ（例「9-8」＝23時間）は書き間違いか別の意味なので採らない。
_OVERNIGHT_MAX_HOURS = 14

# 「24-L」「25時〜ラスト」の "ラスト"（＝閉店まで）。終了時刻は店の営業終了なので
# ここでは決めず、開始だけ確定させて店長に埋めてもらう。区切り記号を必須にして
# いるので、文中の単独の "L" では発火しない。
_WISH_LAST_MARKER_RE = re.compile(
    r"(?<![\d:])(\d{1,2})(?::(\d{2}))?\s*時?\s*[〜～~\-ー―—–]\s*(?:L|ラスト|らすと|閉店)",
    re.IGNORECASE)
...
def _parse_explicit_time_range(text):
    ...
    「22-2」のように終了が開始より小さい書き方は日またぎとして認め、終了に24時を
    足した拡張時刻（"26:00"）で返す。深夜営業の店では最も普通の書き方であり、
    これを弾くと 22:00〜翌2:00 の希望が丸ごと消える。
    """
    ...
    if m:
        try:
            ...
            if 0 <= sh <= 29 and 0 <= eh <= 29:
                if (sh, sm) < (eh, em) and eh - sh <= _OVERNIGHT_MAX_HOURS:
                    return f"{sh:02d}:{sm:02d}", f"{eh:02d}:{em:02d}"
                # 日またぎ（22-2 → 22:00〜26:00）。実働が現実的な長さのときだけ。
                if (sh, sm) > (eh, em) and sh <= 23:
                    dur = (eh + 24) * 60 + em - (sh * 60 + sm)
                    if 0 < dur <= _OVERNIGHT_MAX_HOURS * 60:
                        return f"{sh:02d}:{sm:02d}", f"{eh + 24:02d}:{em:02d}"
        except ValueError:
            pass
    # パターン1b: 「24-L」「22時〜ラスト」。開始だけ確定させる
    m_last = _WISH_LAST_MARKER_RE.search(text)
    if m_last:
        try:
            sh = int(m_last.group(1))
            sm = int(m_last.group(2) or 0)
            if 0 <= sh <= 29:
...
# LLM が返す時刻の検証。"17時" のような値を通すと、サーバ側の norm_hhmm が
# それを黙って "00:00" に潰し、24時間の希望として保存されてしまう。
# 24〜29時は深夜帯の拡張表記として受け付ける（「24-L」＝24時からラストまで）。
# utils.combine_dt_business が営業日基準で翌カレンダー日に落とす。
# 上限は src/app.py の _WISH_TIME_RE と揃える（片方だけ緩めると
# パーサが通した値を保存側が invalid で捨てる）。
_WISH_HHMM_RE = re.compile(r"^([01]?\d|2\d):[0-5]\d$")

# 希望表管理画面の .wmark が理解する語彙（src/app.py の _WISH_AVAILABILITY_VALUES と同一）。
_WISH_AVAILABILITY_VALUES = ("rest", "any", "morning", "evening", "time")

# 深夜帯の書き方をどう返させるか。テキスト版・画像版の両方の system_prompt に入れる。
# 「17日 24-L」は 17日の勤務であり、LLM が気を利かせて 18日 00:00 に直すと、
# 17日の欄に出したい希望が18日に移ってしまう（実際にそう報告された）。
# 日付は書かれた日のまま・時刻は24時超えのまま返させ、実カレンダー日への
# 変換はサーバ側の営業日換算（utils.combine_dt_business）に一本化する。
_WISH_MIDNIGHT_RULE = (
    "深夜帯の扱い（重要）: 「24-L」「25時まで」「26時上がり」のように24時を超える"
    "時刻表記が出てきたら、start/end は 24:00〜29:59 の表記のまま返すこと。"
    "24:00 を 00:00 に直したり、日付を翌日に繰り上げたりしてはいけない。"
    "dates には必ず「その行に書かれている日」を入れること。"
    "「L」「ラスト」は閉店時刻を指す。終了時刻が具体的に書かれていない場合は"
    "end を null にして start だけ返すこと（推測で閉店時刻を作らない）。"
)
...
def parse_wish_text(text, year_month, staff_names=None):
    ...
    system_prompt = (
        ...
        # ★深夜帯の日付★ 「17日 24-L」は 17日の勤務であって 18日の勤務ではない。
        # ここで日付を翌日に繰り上げると、17日の欄に出したい希望が18日に移る。
        # 時刻を 24〜29 時のまま返させ、翌日への変換はサーバ側（営業日換算）に任せる。
        + _WISH_MIDNIGHT_RULE +
        # ★プロンプトインジェクション対策★
